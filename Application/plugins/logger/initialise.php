...
// log cache events
Framework\Events::add(
    "framework.cache.initialize.before",
    function($type, $options) use ($logger) {
...
Framework\Events::add(
    ...
    function($method, $url, $parameters) use ($logger){
...
Framework\Events::add(
    ...
    function($method, $url, $parameters, $response) use ($logger){
...
Framework\Events::add(
    ...
    function($url, $controller, $action, $parameters) use ($logger){
...
Framework\Events::add(
    ...
    function($file, $template) use ($logger){
        $logger->log("framework.view.construct.after: " . $file);
    }
);

Framework\Events::add(
    "framework.view.render.before",function($file) use ($logger){
